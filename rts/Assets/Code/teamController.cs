...
public static class teamController
{
    ...
    public static void registerArea(List<cube> area, teamId t, bool notify = true) {
        if (!ownedTiles.ContainsKey(t)) ownedTiles[t] = new List<cube>();

        // only update if were not notifying
        if (notify) {
            master.localPlayer.syncTeamAffiliation(area, t);
        } else {
